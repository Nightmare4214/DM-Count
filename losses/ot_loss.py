...
class OT_Loss(Module):
    def __init__(self, c_size, stride, norm_cood, device, num_of_iter_in_ot=100, reg=10.0):
        ...
        self.cood = torch.arange(0, c_size, step=stride,
                                 dtype=torch.float32, device=device) + stride / 2  # (1, c_size)
    ...
    def forward(self, normed_density, unnormed_density, points):
        ...
        for idx, im_points in enumerate(points):
            if len(im_points) > 0:  # im_points (M, 2)
                ...
                x = im_points[:, 0].unsqueeze_(1)  # [M, 1]
                y = im_points[:, 1].unsqueeze_(1)  # [M, 1]
                x_dis = -2 * torch.matmul(x, self.cood) + x * x + self.cood * self.cood  # [#gt, #cood]
                y_dis = -2 * torch.matmul(y, self.cood) + y * y + self.cood * self.cood
                y_dis.unsqueeze_(2)
                x_dis.unsqueeze_(1)
                dis = y_dis + x_dis
                dis = dis.view((dis.size(0), -1))  # cost matrix[M, N]

                source_prob = normed_density[idx][0].view([-1]).detach()  # nu(N,)
                target_prob = (torch.ones([len(im_points)]) / len(im_points)).to(self.device)  # mu(M,)
                # use sinkhorn to solve OT, compute optimal beta.
                P, log = sinkhorn(target_prob, source_prob, dis, self.reg, maxIter=self.num_of_iter_in_ot,
                                  log=True)  # P(M, N)
                beta = log['beta']  # beta(N,)
                ot_obj_values += torch.sum(normed_density[idx] * beta.view(
                    [1, self.output_size, self.output_size]))  # <z_hat / ||z_hat||1, beta>
                ...
                source_density = unnormed_density[idx][0].view([-1]).detach()  # (N,)
                ...
                im_grad = im_grad.detach().view([1, self.output_size, self.output_size])  # (1, sqrt(N), sqrt(N))
